...
for (i.switch in 1:9){
  ...
  for (i.tier in 1:2){
    if (i.tier==1) {
      ...
      rel.information <- cbind(i.spillover,i.switch,i.tier,rel.information,treat.size)
...
# Sort data 
    ...
    #main=title,
    pch=c(16),
    col=c("black","grey")[data.tv$i.spillover+1],
    lwd = 1.5)
    ...
    lines(c(i.party,i.party),
          c(data.tv$lower.b[i.party],data.tv$upper.b[i.party]),
                lty=1, lwd=2.5,
          col=c("black","grey")[data.tv$i.spillover[i.party]+1])
    #points(i.party, p.estimate[i.party], col = "black", pch = 21)
    }

    # Add x-axis and y-axis labels
    axis(1, label=F,at=which(data.tv$i.spillover==0))
    axis(2)
    abline(h=0,lty=2)

    data.tv$i.switch[data.tv$i.switch==0] <- NA
    data.tv$i.switch[data.tv$i.spillover==1] <- NA
text(x = 1:nrow(data.tv),
     ...
     ## Increase label size.
     cex = 1.1)
  text(7,0.06,"Effects on FPTP vote share",pos=3)
  text(21,0.06,"Effects on PR vote share",pos=3)
  text(4,0.06,"FPTP candidacy",pos=1)
  text(10,0.06,"On the party list",pos=1)
  text(18,0.06,"On the party list",pos=1)
  text(24,0.06,"FPTP candidacy",pos=1)

  y.pos <- c(-.015,-.02)[data.tv$i.spillover+1]
