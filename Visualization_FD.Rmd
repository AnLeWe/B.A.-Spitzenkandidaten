...
```{r}
data.date_switch <- format(Sys.Date()) # Set to the analysis run date
```

```{r}
all.results <- NULL

file.name.list <- dir("Outputs/Estimation_Results_25/First_Differencing/")

latest_matching_file <- function(pattern, files) {
  matches <- grep(pattern, files, value = TRUE)
  if (!length(matches)) {
    return(NA_character_)
  }
  sort(matches)[length(matches)]
}

for (i.switch in 1:3) { # previously 1:9
  
  for (i.turnout in 0:1) {
    for (i.spillover in 0:1) {
      
      if (i.spillover == 0 & i.turnout == 0) {
        ## baseline: no spillover, no turnout
        file.name <- latest_matching_file(
          paste0("^party\\.selec", i.switch, "_denom1_\\d{4}-\\d{2}-\\d{2}\\.RData$"),
          file.name.list
        )
        if (is.na(file.name)) next
        load(file.path("Outputs/Estimation_Results_25/First_Differencing/", file.name))
      } else {
        ## cases with spillover/turnout
        file.name <- latest_matching_file(
          paste0(
            "^party\\.selec", i.switch,
            "_denom1",
        ...
        if (!is.na(file.name)) {  # only if there are estimation results
          load(file.path("Outputs/Estimation_Results_25/First_Differencing/", file.name))
        } else {
          next  # if there are no estimation results, move to the next loop
        }
      }
...
# Sort data 
    ...
    pdf(file=paste0("Outputs/Estimation_Results_25/First_Differencing/Figures/",title,".pdf"),
        width=8,
        height=5.5,
        pointsize = 13)#,
        #family = "CM Roman")
