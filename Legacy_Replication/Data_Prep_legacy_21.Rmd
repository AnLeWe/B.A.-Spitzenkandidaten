...
This document contains the R code used to combine the individual csv files from the Federal Returning Officer, which contain the polling-station and postal-vote results by electoral district for the Bundestag elections from 2002 to 2021, into one higher-level dataset. The corresponding csv files are publicly available on the Federal Returning Officer's website (tables: final results by electoral district for all Bundestag elections, https://www.bundeswahlleiter.de/bundestagswahlen/2021/ergebnisse.html, accessed on 13.12.2022).

The final results by federal state are also provided. These were accessed on ....


## Separate datasets by election year
...
# read in results # AFD to be added
...
## Pooling the individual datasets by election year

Current and previous election results are pooled separately.
...
save(data_now, file = "../Data/data_now_legacy21.RData")
save(data_then, file = "../Data/data_then_legacy21.RData")
...
```{r}
library(readxl)
treat.path <- "../Data/Treatment_WK_pre2025_from_git.xlsx"
treat.raw <- read_xlsx(treat.path,sheet=2)
...
for (i in 1:nrow(treat.raw)){# Loop over all rows
  
  # Only keep the rows for the Bundestag elections we want to include
...
An array of absolute election results with 5 dimensions is created:
 ** Electoral districts (1 ... 299)
 ** Parties (1... 6): C S A F L G
 ** First or second vote (1, 2)
 ** Year (1 .. 6) 2021 2017 2013 2009 2005 2002
 ** Current or previous results (1,2)

An array of eligible voters and valid votes with 5 dimensions is created:
 ** Electoral districts (1 ... 299)
 ** Eligible voters, voters, or valid votes (1 .. 3)
 ** First or second vote (1, 2)
 ** Year (1 .. 6) 2021 2017 2013 2009 2005 2002
 ** Current or previous results (1,2)


An array of treatments with four dimensions is created:
 ** Electoral districts (1 ... 299)
 ** Parties (1... 6): C S A F L G
 ** First or second vote (1, 2)
 ** Year (1 .. 6) 2021 2017 2013 2009 2005 2002
...
A matrix of electoral district names
...
for (i.year in 1:6){
  for(i.now.then in 1:2){
    ...
    # Eligible voters, voters, valid votes
     # Erststimme
    ...
    # Eligible voters, voters, valid votes
     ...
     all.abs.total[,,2,i.year,i.now.then] <- as.matrix(temp.dat[,c(4,5,9)])
         ...
         # For the 2002 Bundestag election we do not need previous results
         next
       }
     }
...
District assignment for fixed-effect models
...
for (i.party in 1:dim(all.treat)[2]){
  for (i.smdpr in 1:2){
...
save(data_now,
     ...
     all.wkr.index,
     all.land,
  file = "../Data/analysis_dat_legacy21.RData")
