...
for (i.smdpr in 1:2){ # Loop over Erst- and Zweitstimme
  ...
  print(summary(lm.out1 <- lm(votes~ treat + year * party, data=lm.dat)))

  print(c("Robust standard error"))
  r.se1 <- sqrt(diag(vcovHC(lm.out1, type = 'HC')))
  print(coef.rb1 <- coeftest(lm.out1, vcov = vcovHC(lm.out1, type = 'HC')))

  all.results[[1]] <- lm.out1
  
  ## Model with cross treatment (contamination)

  print(c("Contamination on Erststimme (H2b)","Contamination on Zweitstimme  (H2a)")[i.smdpr])
  print(summary(lm.out2 <- lm(votes~ treat + cross.treat + year * party, data=lm.dat)))
  
  print(c("Robust standard error"))
  r.se2 <- sqrt(diag(vcovHC(lm.out2, type = 'HC')))
  print(coef.rb2 <- coeftest(lm.out2, vcov = vcovHC(lm.out2, type = 'HC')))
  
 
       
  all.results[[2]] <- lm.out2
...
dat.pr <- all.results.pr[[3]]
dat.smd <- all.results.smd[[3]]
```

```{r}
 #stargazer(all.results[[2]], coef.rb, type = "text")
```
