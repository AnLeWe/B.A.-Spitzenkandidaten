...
if (party.selec.switch==4) party.selection <- c(3:4,6)    # ohne C, S und L
if (party.selec.switch==5) party.selection <- c(2:6)      # ohne C
if (party.selec.switch==6) party.selection <- c(1,3:6)    # ohne S
if (party.selec.switch==7) party.selection <- c(1:4,6)    # ohne Linke
if (party.selec.switch==8) party.selection <- c(1:2,4:6)  # ohne AfD
if (party.selec.switch==9) party.selection <- c(2,4:6)    # ohne C und A
if (party.selec.switch==10) party.selection <- c(1:3,5:6) # ohne F
if (party.selec.switch==11) party.selection <- c(1:5)     # ohne G
```
...
for (i.smdpr in 1:2){ # Loop over Erst- and Zweitstimme
  ...
  lm.dat <- cbind(votes,treat,cross.treat,treat.incumb)
  ...
  print(summary(lm.out <- lm(votes~ treat +
                                  # + treat.incumb                                 # + robustness-check
                                    cross.treat + year * party, data=lm.dat))) 
...
for (i.smdpr in 1:2){ # Loop over Erst- and Zweitstimme
  ...
  all.t1.results <- vector("list",3) # empty list to store the results
  ...
  treat.incumb.t1 <- all.treat.incumb.fd[,,i.smdpr,1:4] 
  ...
  lm.dat <- cbind(votes,treat,year,party,
                  cross.treat,treat.incumb,
                  treat.t1,cross.treat.t1,treat.incumb.t1)
  lm.dat <- as.data.frame(lm.dat)
  ...
  print(summary(lm.out <- lm(votes~ treat + treat.t1+
                                     ...
                                     #treat.incumb + treat.incumb.t1 +
                               as.factor(year) * as.factor(party),
                             data=lm.dat)))
  ...
  all.t1.results[[1]] <- lm.out

  all.t1.results[[2]] <- lm.out.robust
  
  all.t1.results[[3]] <- lm.dat
  all.t1.results[[4]] <- votes.in.array
  
  all.t1.results[[5]] <- r.se
  
  if (i.smdpr==1) all.t1.results.smd <- all.t1.results
  if (i.smdpr==2) all.t1.results.pr <- all.t1.results
}

save(all.t1.results.smd,
     all.t1.results.pr,
