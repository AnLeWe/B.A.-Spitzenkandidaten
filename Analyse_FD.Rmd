...
for (i.smdpr in 1:2){ # Loop over Erst- and Zweitstimme
  ...
  print(coef.rb <- coeftest(lm.out, vcov = vcovHC(lm.out, type = 'HC')))
  #coeftest(lmfit, vcov = vcovHC(lmfit))
  models <- list(lm.out, coef.rb)
  
  # combined table
  
  print(c("Contamination on Erststimme (H2b)","Contamination on Zweitstimme  (H2a)")[i.smdpr])
  stargazer(lm.out, coef.rb, title="Ergebnisse", type = "text",align=TRUE,
            star.cutoffs = c(0.05,0.01,0.001))
  stargazer(lm.out, coef.rb, title="Ergebnisse", se=list(NULL, coef.rb), align=TRUE,
            star.cutoffs = c(0.05,0.01,0.001))
 
  #print(modelsummary(models, output = "latex"))
  #xtable(anova(lm.out, coef.rb))
  #print(xtable(lm.out, caption = Hier wird einmal eine Caption stehen))
