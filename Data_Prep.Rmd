...
for (i.year in 1:7){ # 2025, 2021, 2017, 2013, 2009, 2005, 2002
  for(i.now.then in 1:2){ # now (1) or then (2)
    ...
    if (i.year < 7){
      ...
      if (i.now.then==1){
        ...
        as.matrix(temp.dat[,c(10,12,14,16,23)])  # S C G F A L B

        # Zweitstimme  
        all.abs.votes[,c(1,2,3,4,6), 2, i.year, i.now.then] <-
        as.matrix(temp.dat[,c(11,13,15,17,24)])  # S C G F A L B
  ...
  temp.treat <- data_treat[(1+299*(i.year-1)):(299*i.year),] # 2025 for i.year=1, 2021 for i.year=2, ...1
  # for i.year = 6, it's row 1496:1794, is it not? which corresponds to 2005. And 2002 is 7, is 1795:2093, which is the 2002 treatment.
  # Treatment
  # First vote - 7 parties
  all.treat[,,1,i.year] <- 
    as.matrix(temp.treat[,seq(12, 24, by=2)]) # 24 is letzte Erststimme
  # Second vote - 7 parties
  all.treat[,,2,i.year] <-
    as.matrix(temp.treat[,seq(13, 25, by=2)]) # 25 is letzte zweitstimme
}
