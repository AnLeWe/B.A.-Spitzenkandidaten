...
result0202 <- result0202[, names(result0202) != "Land "]  # Drop column with space
...
rbind.with.diff.col <- function(...) {
  dfs <- list(...)
  
  all_cols <- unique(unlist(lapply(dfs, names)))
  
  pad_and_order <- function(df) {
    missing_cols <- setdiff(all_cols, names(df))
    if (length(missing_cols) > 0) {
      df[missing_cols] <- NA
    }
    return(df[, all_cols, drop = FALSE]) # Return with columns in the exact same order
  }
  
  do.call(rbind, lapply(dfs, pad_and_order))
}

data_now <- rbind.with.diff.col(result2525,result2121,result1717,result1313,result0909,result0505)
data_then <- rbind.with.diff.col(result2521,result2117, result1713,result1309,result0905,result0502)

# Some manual correction of the structure
result0202_temp <- result0202[,c(1,2,20,3:19)]
names(result0202_temp) <- names(result0505)[c(1:5,8:22)]


data_now_with_2002 <- 
  rbind.with.diff.col(result2525,result2121,result1717,result1313,result0909,result0505,result0202_temp)
...
colnames(result0202)[c(1:2,3:4)] <- colnames(data_now)[c(1:2,4,6)]
colnames(result0202)[c(17,18)] <- colnames(data_now)[c(22,23)]

result0202[setdiff(names(data_now), names(result0202))] <- NA # adds empty cols for eligible/turnout 2nd
data_treat <- rbind(data_treat,result0202) # full df with results from 2002 to 2025
...
for (i in 1:nrow(treat.raw)){
  ...
  # Only rows for BTW we want to include
  if (as.numeric(treat.raw$Jahr[i]) %in% all.years){
    ...
    which.land <- which(all.states == treat.raw$Land.neu[i]) # e.g. row 2: NRW -> 5
...
```{r}
# Build data_treat_then: previous-year treatment in current-year row ordering.
#
# Two types of cross-year territory correspondence exist:
#
#   1. Stable renumbering: same geographic territory, WK number changed between
#      elections (e.g. Trittin WK54→53, Gysi WK85→84). The Nr.XX canonical IDs
#      in data_now_WK_Einteilung.xlsx sheet 6 are stable for these cases:
#      Nr.XX[i] == Nr.YY[j] identifies the predecessor row j. Using the same
#      row index i would pick the wrong territory.
#
#   2. Boundary change: the territory was redefined (minor boundary adjustment)
#      and received a new canonical ID. The Nr.XX key returns NA for these rows.
#      The WK number itself is stable (Nr.XX.orig == 1:299 for all years), so
#      the direct row fallback (same row index = same WK number) gives the
#      correct predecessor (e.g. Merkel CDU WK15: ran there in both 2009 and
#      2013 despite canonical ID change 15→330).
#
# Hybrid rule: canonical match where available, direct row fallback for NA keys.

nr_cols <- as.matrix(
  read_xlsx("Data/data_now_WK_Einteilung.xlsx", sheet = 6)[,
    paste0("Nr.", c("25", "21", "17", "13", "09", "05", "02"))]
)

data_treat_then <- data_treat

for (t in 1:6) {
  curr_rows <- (1 + 299*(t-1)):(299*t)
  prev_rows <- (1 + 299*t):(299*(t+1))

  key     <- match(nr_cols[, t], nr_cols[, t + 1])
  matched <- !is.na(key)

  # canonical predecessor for stable/renumbered territories
  data_treat_then[curr_rows[matched],  ] <- data_treat[prev_rows[key[matched]], ]
  # direct row (same WK number) for boundary-change territories
...
`all.abs.votes`: An array of absolute election results with five dimensions is created:
...
`all.abs.total`: An array of eligible voters and valid votes with five dimensions is created:
...
`all.treat`: An array of the treatment with five dimensions is created:

* **Constituencies** (1 … 299)
* **Parties** (1 … 7): S C G F A L B
* **First or second vote** (1, 2)
* **Year** (1 … 7): 2025, 2021, 2017, 2013, 2009, 2005, 2002
* **Current or previous results** (1, 2) — dim 5=2 holds previous-year treatment reordered to current-year row positions via `data_treat_then`

A matrix of constituency names (299 rows × 7 years)

A matrix of federal states (299 rows × 7 years)

## Data Structure Notes

**Party ordering across all arrays:** SPD, CDU/CSU, GRÜNE, FDP, AfD, DIE LINKE, BSW (positions 1-7)

**`data_now`, `data_then`, and `data_treat`:**
- All have identical column structures
- Columns 12-25: All party data in the order S, C, G, F, A, L, B (odd columns = Erststimmen, even columns = Zweitstimmen)

**`result0202`:**
- Same party order: S, C, G, F, A, L, B
- Parties A (AfD) and B (BSW) have empty/NA values (parties didn't exist in 2002)
- Different column positions for some electoral totals:
  - "Wahlberechtigte Zweitstimmen" in column 21 (instead of column 5)
  - "Waehlende Zweitstimmen" in column 22 (instead of column 7)

**Missing data by design:**
- 2002 (year 7): AfD (position 5) and BSW (position 7) are NA in `all.abs.votes`
- 2002 (year 7): Only current results exist (i.now.then = 1); previous results (i.now.then = 2) are NA
- BSW (position 7): Only exists for 2025 (year 1); all other years are NA
- Earlier years: AfD may be NA for years before 2013 when the party was founded



```{r}
all.abs.votes <- array(NA, dim=c(299, 7, 2, 7, 2))  # 7 parties, 7 years
all.abs.total <- array(NA, dim=c(299, 3, 2, 7, 2))  # 7 years
all.treat     <- array(NA, dim=c(299, 7, 2, 7, 2))  # 7 parties, 7 years, current/previous
all.wkr.name <- matrix(NA, nrow=299, ncol=7)        # 7 years
all.land <- matrix(NA, nrow=299, ncol=7)            # 7 years

for (i.year in 1:7){ # 2025, 2021, 2017, 2013, 2009, 2005, 2002
  for(i.now.then in 1:2){ # now (1) or then (2)
    if (i.now.then==1){
      temp.dat <- data_now
    } else {
      temp.dat <- data_then
    ...
    # Federal elections 2025 - 2005
    if (i.year < 7){
      temp.dat <- temp.dat[(1+299*(i.year-1)):(299*i.year),] 

      # Election results   
      # First vote (direct candidate) - now 7 parties (columns 12-24, by=2)
      all.abs.votes[,,1,i.year,i.now.then] <-
        as.matrix(temp.dat[,seq(12, 24, by=2)]) # S C G F A L B
      # Second vote (party list) - now 7 parties (columns 13-25, by=2)
      all.abs.votes[,,2,i.year,i.now.then] <-
        as.matrix(temp.dat[,seq(13, 25, by=2)]) # S C G F A L B

      # Eligible voters, Turnout, Valid votes
      # First vote
      ...
      # for 2002 federal election
      if (i.now.then==1){
        temp.dat <- result0202
        
        # Election results 2002
        # 2002 doesn't have AfD (position 3) or BSW (position 7)
        
        # Erststimme
        all.abs.votes[,c(1,2,3,4,6), 1, i.year, i.now.then] <- 
        as.matrix(temp.dat[,c(9,11,13,15,17)])  # S C G F L

        # Zweitstimme  
        all.abs.votes[,c(1,2,3,4,6), 2, i.year, i.now.then] <-
        as.matrix(temp.dat[,c(10,12,14,16,18)])  # S C G F L
        
        # Eligible voters, Turnout, Valid votes
        # First vote
        all.abs.total[,,1,i.year,i.now.then] <- as.matrix(temp.dat[,c(3,4,7)])
        # Second vote
        all.abs.total[,,2,i.year,i.now.then] <- as.matrix(temp.dat[,c(3,4,8)])

      } else {
        # For 2002 federal election we don't need previous results
        next
      }
    }
    ...
    # NOTE: for year 7 (2002), temp.dat = result0202 which has no Land column.
    # Col 3 of result0202 is Wahlberechtigte, not the state index (1–16).
    # all.land[,7] therefore contains eligible voter counts (~150k–250k) instead
    # of state indices. No current analysis uses all.land[,7]; if 2002 state
    # assignments are ever needed, fix by reading the Land column from the 2002
    # source data explicitly rather than relying on col 3 here.

    # Treatment (mirrors vote filling: i.now.then=1 from data_treat, 2 from data_treat_then)
    temp.treat <- if (i.now.then == 1) data_treat else data_treat_then
    temp.treat  <- temp.treat[(1 + 299*(i.year-1)):(299*i.year), ]
    all.treat[,,1, i.year, i.now.then] <- as.matrix(temp.treat[, seq(12, 24, by=2)])
    all.treat[,,2, i.year, i.now.then] <- as.matrix(temp.treat[, seq(13, 25, by=2)])
  }
}
...
```{r}
# Check that 2002 AfD and BSW are NA
sum(!is.na(all.abs.votes[, 5, , 7, 1]))  # Should be 0 (AfD 2002)
sum(!is.na(all.abs.votes[, 7, , 7, 1]))  # Should be 0 (BSW 2002)

# Check that 2002 previous results are all NA
sum(!is.na(all.abs.votes[, , , 7, 2]))   # Should be 0

# Check BSW only exists in 2025
sum(!is.na(all.abs.votes[, 7, , 1, 1]))  # Should > 299, only some direct candidates  (2025 BSW)
sum(!is.na(all.abs.votes[, 7, , 2:7, 1])) # Should be 0 (earlier years)

# 2025 (year 1) - rows 1-299 in data_now
all.abs.votes[1, 1, 2, 1, 1]
data_now[1, "SPD Zweitstimmen"]

# 2021 (year 2) - rows 300-598 in data_now
all.abs.votes[1, 1, 2, 2, 1]
data_now[300, "SPD Zweitstimmen"]

# 2017 (year 3) - rows 599-897 in data_now
all.abs.votes[1, 1, 2, 3, 1]
data_now[599, "SPD Zweitstimmen"]

# Check treatment for Fischer 2002 (constituency 184, GRUENE=position 3, Erststimmen)
all.treat[184, 3, 1, 7, 1]  # Should be 6 (dim5=1: current)

# Check dimensions are correct
dim(all.abs.votes)  # Should be: 299 7 2 7 2
dim(all.abs.total)  # Should be: 299 3 2 7 2
dim(all.treat)      # Should be: 299 7 2 7 2
```


Constituency allocation for fixed-effect models

```{r}
wkr.assignment <- read_xlsx("Data/data_now_WK_Einteilung.xlsx",sheet=6)

wkr.assignment <- wkr.assignment[,paste0("Nr.",c("25","21","17","13","09","05","02"))] # timetraced wkr
wkr.assignment <- as.matrix(wkr.assignment)


all.wkr.index <- all.treat[,,,,1]  # [299, 7, 2, 7] — 4D, current-year only
for (i.party in 1:length(all.parties2)){
  for (i.smdpr in 1:2){
...
file.names <- dir("BTW_Strukturdaten") # 02, 05, 09, 13, 17, 21, 25

list.struktur <- vector("list",length(file.names))

## How many lines to skip to obtain the header
to.skip.header <- c(3,3,3,4,8,8,9) # <- added 2025, but not readable
lines.header <- c(2,2,3,1,1,1,1)
to.skip.data <- c(5,5,7,6,9,9,10)
...
```{r}
library(haven)
filenames <- list.files("Data/politbarometer/", pattern = "*.sav", full.names = TRUE)
polit_list <- setNames(lapply(filenames, read_sav), basename(filenames))
...
save(data_now,data_now_with_2002,
     data_then,
     data_treat,
     all.abs.votes,
     all.abs.total,
